...
const contactInfo = [];
for(i = 0; i < graduates.length; i++){
  contactInfo.push(`Student name: ${graduates[i].first_name}, Email: ${graduates[i].email}`);
}
...
const result = zooAnimals.filter(item => item.population <= 5);
console.log(result);
...
const x = zooAnimals.reduce((accumulator, animal) => accumulator + animal.population, 0);
console.log(x);
